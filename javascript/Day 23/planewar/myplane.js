...
Myplane.prototype.myplaneMove = function() {
    var that = this;
    this.initx = (myplanebox.offsetWidth - this.w) / 2;
    this.inity = myplanebox.offsetHeight - this.h;
    this.maxleft = myplanebox.offsetWidth - this.w;
    this.maxtop = myplanebox.offsetHeight - this.h;
    document.addEventListener('keydown', keymove, false);
    var uptimer, downtimer, lefttimer, righttimer;
    var uplock = true,
        downlock = true,
        leftlock = true,
        rightlock = true;
        ...
        function moveup() {
            if (uplock) {
                ...
                uptimer = setInterval(function() {
                    ...
                    that.imgmyplane.style.top = that.y + 'px';
                }, 20);
            }
        }
        ...
        function moveleft() {
            if (leftlock) {
                ...
                lefttimer = setInterval(function() {
                    ...
                    that.imgmyplane.style.left = that.x + 'px';
                }, 20);
            }
        }
...
Myplane.prototype.myplaneshoot = function() {
    ...
    document.addEventListener('keydown', shootkey, false)
    var shootlock = true;

    function shootkey(ev) {
        var ev = ev || window.event;
        if (ev.keyCode == 74) {        //j键发射
            if (shootlock) {
                shootlock = false;
                shootbullet();
    ...
    this.unbindshootkey=function(){
    	clearInterval( that.shoottimer);
    	document.removeEventListener('keydown', shootkey, false)
    }
    document.addEventListener('keyup', function(ev) {
        var ev = ev || window.event;
        if (ev.keyCode == 75) {
...
Enemy.prototype.enemyHit = function() {
    if ((this.x + this.w) >= ourplane.x && this.x <= (ourplane.x + ourplane.w) && (this.y + this.h) >= ourplane.y && this.y <= (ourplane.y + ourplane.h)) {
        ...
        ourplane.unbindmovekey();
        ourplane.unbindshootkey();
